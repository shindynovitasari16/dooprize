 <?php
    ...
    elseif(isset($_POST['update'])){
      ...
      $sql = "UPDATE users SET nama=?, alamat=?, meta=?, ikut=? WHERE user_id='$id'";
      $pre = $conn->prepare($sql);
      $pre->bind_param("ssi", $nama, $alamat, $kelas, $ikut);
       if($pre->execute()){
        echo "Disimpan";
       } else {
        echo "Gagal";
    ...
    // Tampilan default Mengacak nama pemenang yang di ambil dari database
        ...
        if ($query->num_rows > 0){
            ...
            $dayLimit = '0'; // Hanya dapat melakukan pengoclokan (n) Hari sekali
